...
def on_tab_change(event, ui_elements, global_state, ui_callbacks):
    ...
    try:
        ...
        if selected_tab_text == 'Cadastro':
            ...
            ui_elements.get('entry_nome_cadastro').focus_set()
            ...
            ui_elements.get('entry_ncliente_creditos').focus_set()
        elif selected_tab_text == 'Saída':
            ui_elements.get('entry_pesquisa_cliente_saida').focus_set()
        elif selected_tab_text == 'Entrada':
            ui_elements.get('entry_pesquisa_remetente').focus_set()
        elif selected_tab_text == 'Programado':
            frame = ui_elements.get('scrollable_frame_prog')
            if frame:
                for widget in frame.winfo_children():
                    widget.destroy()
            ui_elements.get('entry_pesquisa_remetente_programado').focus_set()
...
def refresh_programados_e_invalidar_cache(ui_elements, global_state, ui_callbacks, show_message_func):
    ...
    # Isso garante que novos clientes ou alterações apareçam na lista de remetentes.
    ui_callbacks['invalidar_cache_clientes']()
    print("[CACHE] Cache de clientes invalidado manualmente pelo botão Atualizar.")
    
    # Passo 2: Chama a função de busca original, que agora lerá os dados mais recentes do DB.
    # A função buscar_programados_ui já lê a tabela 'programado' diretamente,
    # então a combinação das duas ações resolve o problema.
...
def buscar_programados_ui(ui_elements, global_state, ui_callbacks, show_message_func):
    ...
    for i, prog_data in enumerate(programados):
        # Cria um Frame para cada linha para melhor separação e encapsulamento
        row_frame = ttk.Frame(frame_lista, padding=5, relief="solid", borderwidth=1)
        row_frame.pack(fill='x', expand=True, pady=5, padx=5)

        # Configuração das colunas dentro do frame da linha
        row_frame.grid_columnconfigure(0, weight=3)  # Info
        row_frame.grid_columnconfigure(2, weight=1)  # Modalidade
        row_frame.grid_columnconfigure(4, weight=1)  # Pacote
        row_frame.grid_columnconfigure(5, weight=0)  # Botão
        
        # <<< INÍCIO DA ALTERAÇÃO >>>
        # Coluna 0: Informações de Data e Destinatário
        data_hora_val = prog_data.get('data_hora_programado')
        data_hora_obj = None

        if isinstance(data_hora_val, str):
            try:
                # Tenta converter o texto (string) para um objeto de data/hora
                data_hora_obj = datetime.datetime.strptime(data_hora_val, "%Y-%m-%d %H:%M:%S")
            except (ValueError, TypeError):
                # Se a conversão falhar, usa a data e hora atuais como padrão
                data_hora_obj = datetime.datetime.now()
        elif isinstance(data_hora_val, datetime.datetime):
            # Se já for o objeto correto, apenas o utiliza
            data_hora_obj = data_hora_val
        else:
            # Para qualquer outro caso (None, etc.), usa a data e hora atuais
            data_hora_obj = datetime.datetime.now()

        # Agora, com certeza temos um objeto de data/hora válido para formatar
...
def dar_baixa_envelopes(ui_elements, global_state, ui_callbacks, show_message_func):
    ...
    for codigo in codigos_para_baixa:
        envio_original = next((env for env in global_state['envios_exibidos_na_saida'] if env.get('codigo') == codigo), None)
        if not envio_original: continue

        dados_saida = {
            **envio_original,
            'codigo_envelope': envio_original.get('codigo'),
            'data_hora_registro_original': envio_original.get('data_hora_registro').strftime("%Y-%m-%d %H:%M:%S") if isinstance(envio_original.get('data_hora_registro'), datetime.datetime) else envio_original.get('data_hora_registro'),
    ...
    if sucessos > 0 and destinatario_telefone_final and destinatario_telefone_final != "N/A":
        evento_bot = {
...
def selecionar_cliente_para_edicao(event, ui_elements):
    ...
    if values and len(values) >= 3:
        ...
        ui_elements['entry_ncliente_cadastro'].delete(0, tk.END); ui_elements['entry_ncliente_cadastro'].insert(0, values[2])
        ui_elements['entry_nome_cadastro'].focus_set()
...
def aplicar_filtros_hist_entrada(ui_elements, ui_callbacks):
    ...
    rem_filter_nome = ui_elements.get('combo_cliente_hist_entrada').get().split(' - ')[0].strip()
    dest_filter_nome = ui_elements.get('combo_dest_hist_entrada').get().split(' - ')[0].strip()
    ...
    ui_helpers.exibir_historico(ui_elements.get('tree_historico'), 'entradas', ui_callbacks, 
...
def aplicar_filtro_cliente_hist_saida(ui_elements, ui_callbacks):
    ...
    cli_filter_nome = ui_elements.get('combo_cliente_hist_saida').get().split(' - ')[0].strip()
    ...
    ui_helpers.exibir_historico(ui_elements.get('tree_historico_saida'), 'saidas', ui_callbacks, 
